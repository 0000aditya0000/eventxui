...
const UserProfile = () => {
  ...
  const validateForm = () => {
    ...
    const isFormComplete = name !== '' && email !== '' && phone !== '' && age !== '';

    setIsFormValid(isEmailValid && isPhoneValid && isAgeValid && isFormComplete);
  };
  ...
  const handleImagePicker = async () => {
    ...
    if (!pickerResult.canceled) {
      ...
      reader.readAsDataURL(await fetch(pickerResult.assets[0].uri).then(res => res.blob()));
      reader.onloadend = () => {
    ...
    <ScrollView showsVerticalScrollIndicator={true}>
      <View style={styles.container}>
        <View style={styles.avatarWrapper}>
          <TouchableOpacity onPress={handleImagePicker}>
            <Avatar
              rounded
              size="xlarge"
              source={profilePicture ? { uri: `data:image/jpeg;base64,${profilePicture}` }  : defaultAvatar}
            />
            <Ionicons name="add-circle" size={23} color="#d6001c" style={styles.iconContainer} />
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.btnBox}>
          <TouchableOpacity
            style={[styles.btnContainer, !isFormValid && styles.disabledBtnContainer]}
            onPress={handleEdit}
